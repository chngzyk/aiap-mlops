...
(`src/aiap_mlops/general_utils.py`)
...
a Dockerfile (`docker/aiap-mlops-data-prep.Dockerfile`)
provided in this template:

=== "Linux/macOS"

    ```bash
    $ export GCP_PROJECT_ID=machine-learning-ops
    $ docker build \
        -t asia.gcr.io/$GCP_PROJECT_ID/data-prep:0.1.0 \
        -f docker/aiap-mlops-data-prep.Dockerfile .
...
=== "Windows PowerShell"
    ...
    ```powershell
    $ $GCP_PROJECT_ID='machine-learning-ops'
    $ docker build `
        -t asia.gcr.io/$GCP_PROJECT_ID/data-prep:0.1.0 `
        -f docker/aiap-mlops-data-prep.Dockerfile .
    $ docker push asia.gcr.io/$GCP_PROJECT_ID/data-prep:0.1.0
...
=== "Linux/macOS"
    ...
    $ polyaxon run -f aisg-context/polyaxon/polyaxonfiles/process-data.yml \
        ...
        -P WORKING_DIR="/home/aisg/aiap-mlops" \
...
In the module `src/aiap_mlops/general_utils.py`,
...
!!! important
    ...
    following name: `gs://aiap-mlops-artifacts`.
    ...
    will be
    `gs://aiap-mlops-artifacts/mlflow-tracking-server`.

=== "Local Machine / Polyaxon VSCode Terminal"

    ```bash
    $ gsutil mb -p machine-learning-ops -c STANDARD -l ASIA-SOUTHEAST1 -b on gs://aiap-mlops-artifacts
    Creating gs://aiap-mlops-artifacts/...
    ```
...
=== "Linux/macOS"
    ...
    $ docker build \
        ...
        -f docker/aiap-mlops-model-training-gpu.Dockerfile .
