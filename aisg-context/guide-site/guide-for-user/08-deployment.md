...
- Artifacts for specific runs will be uploaded to a directory with the
  ...
  `gs://aiap-mlops-artifacts/mlflow-tracking-server/<MLFLOW_EXPERIMENT_UUID>`.
...
=== "Linux/macOS"
    ...
    $ cd src
    $ gunicorn aiap_mlops_fastapi.main:APP -b 0.0.0.0:8080 -w 4 -k uvicorn.workers.UvicornWorker
    ```
...
=== "Windows PowerShell"
    ...
    $ conda activate aiap-mlops
    $ cd src
    $ uvicorn aiap_mlops_fastapi.main:APP
    ```
...
`src/aiap_mlops_fastapi/config.py`. This class contains
...
`src/aiap_mlops_fastapi/config.py`:
```python
...
class Settings(pydantic.BaseSettings):

    API_NAME: str = "aiap_mlops_fastapi"
...
=== "Linux/macOS"
    ...
    $ docker build \
        ...
        -f docker/aiap-mlops-fastapi.Dockerfile .
    ```
