...
ARG REPO_DIR="."
ARG CONDA_ENV_FILE="aiap-mlops-conda-env.yml"
ARG CONDA_ENV_NAME="aiap-mlops"
...
ARG PRED_MODEL_GCS_URI="gs://aiap-mlops-artifacts/mlflow-tracking-server/$PRED_MODEL_UUID"
...
RUN $CONDA_BIN env create -f aiap-mlops/$CONDA_ENV_FILE && \
    $CONDA_BIN init bash && \
    $CONDA_BIN clean -a -y && \
    echo "source activate $CONDA_ENV_NAME" >> "$HOME_DIR/.bashrc"

WORKDIR $HOME_DIR/aiap-mlops
